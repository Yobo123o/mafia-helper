...
exports.getInitialRoleStates = getInitialRoleStates;
exports.buildWakeOrder = buildWakeOrder;
exports.validateAction = validateAction;
const roles_1 = require("./roles");
function getInitialRoleStates() {
    return {
        Civilian: {},
        Detective: {},
        Doctor: {},
...
function buildWakeOrder(rolesInPlay, options) {
    const { nightNumber, rolesAlive } = options;
    const isAlive = (role) => rolesAlive?.[role] !== false;
    const wakeOrder = [];
    if (rolesInPlay.includes("Cupid") && nightNumber === 1 && isAlive("Cupid")) {
        wakeOrder.push("Cupid");
    }
    const night1OnlyWakeRoles = new Set(["Postman", "Grandma"]);
    for (const role of roles_1.WAKE_ORDER) {
        if (!rolesInPlay.includes(role))
            continue;
        if (!isAlive(role))
            continue;
        if (night1OnlyWakeRoles.has(role) && nightNumber !== 1)
            continue;
        wakeOrder.push(role);
    }
    return wakeOrder;
}
function validateAction(role, targetIds, context, metadata) {
    const definition = roles_1.ROLE_DEFINITIONS[role];
